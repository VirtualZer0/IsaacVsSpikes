<template>
  <div class="editor-tag-list">
    <div class="editor-tag-list-item" v-for="(tag, index) in curTags" :key="index" :style="{backgroundColor: getColor(tag)}">
      <div class="editor-tag-list-item-name">{{ tag }}</div>
      <div class="eui button editor-tag-list-item-remove" @click="curTags.splice(index, 1)">
    ...
    <div class="editor-tag-list-new">
      <input type="text" class="eui input small" v-model="newTag" @keypress.enter="addTag()"/>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    return {
      curTags, newTag, getColor, addTag
    };
  }
});
