<template>
  <div class="editor-link-list">
    <editor-link v-for="(res, num) in curLinks" :key="num" :res="res" :type="type" @select="curLinks[num] = $event"
      @remove="curLinks.splice(num, 1)" :spriteMode="spriteMode" />
    <editor-link :type="type" @select="curLinks.push($event)" :spriteMode="spriteMode" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    const newRes = props.spriteMode ? ref<SpriteSource | null>(null) : ref<ResourceLink | null>(null);
