  ...
  <div class="editor-locale-input quill">
    ...
    <div :id="customToolbarId+num" class="quill-custom-toolbar eui" v-for="(_b, num) in text" :key="num" v-show="activeEditor == num">
      <div>
        ...
        <select class="ql-color" :key="num">
        </select>
        <select class="ql-background" :key="num">
        </select>
      </div>
    ...
    <div class="text-block" v-for="(blocks, num) in text" :key="num">
      ...
      <div class="text-block-controls">
        <svg style="width:24px;height:24px" viewBox="0 0 24 24" @click="activeEditor = num - 1; num == 1 || removeBlock(num)">
      ...
      <QuillEditor
        ...
        :ref="setEditor"
        theme="snow"
        :content="copyText[num] && copyText[num][currentLocale] ? copyText[num][currentLocale] : ''"
...
import { defineComponent, PropType, ref, reactive, onBeforeUpdate, watch} from 'vue'
