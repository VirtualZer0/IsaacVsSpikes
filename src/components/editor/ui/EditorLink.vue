<template>
  <div class="editor-link" :class="{selected: res != null}" @click="showModal = true">
    <teleport to="body" v-if="showModal">
      <editor-modal :title="$t(`editor.${type}`)" @close="showModal = false">
        <resources-list :resourceType="type" include-builtin select-mode @select="selectResource" />
      </editor-modal>
    </teleport>
    ...
    <editor-res-preview :res="resource" v-if="res" class="preview" :height="80"/>
    <div class="name" v-if="res">
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    const resource = computed(() => {
      ...
      const propRes = props.spriteMode ? (props.res as SpriteSource).src as ResourceLink : props.res as ResourceLink;
