...
export async function runMigration(from: number, to: number, fs: IBaseFS) {
  ...
  alert('Current project version is outdated. Please wait while we migrate your project to the latest version.');
  ...
  await (await fs.getFile('project.json')).writeAllText(JSON.stringify({
    name: editor.name,
    uuid: editor.uuid,
    version: to
  }))
...
const log = (type: 'log' | 'warn' | 'error', ...args: any[]) => {
  console[type]('%cProjectMigration', 'background-color: #FF9800; color: #333; border-radius: 100px;padding: 1px 4px', ...args);
}